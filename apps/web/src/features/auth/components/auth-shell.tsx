...
interface AuthShellProps {
  ...
  hideLogo?: boolean;
  /** Ключ анимации — при смене карточка «въезжает» заново. */
  animKey?: string;
}

/** Полноэкранный слой со свечением, карточка по центру, переключатель темы справа сверху. */
export function AuthShell({ children, wide, foot, hideLogo, animKey }: AuthShellProps) {
  return (
...
/** Заголовок карточки + подзаголовок (h2 20px / sub 13px) — как в демо. */
...
/** Ссылки под формой: «Забыл пароль?» / «← Назад» и приглушённая подпись справа. */
