...
export const authInputClass =
  'h-auto rounded-[10px] border-border bg-surface-2 px-3 py-[11px] text-sm text-foreground placeholder:text-text-3 dark:bg-surface-2 focus-visible:border-brand focus-visible:ring-3 focus-visible:ring-brand-soft aria-invalid:border-crit aria-invalid:ring-crit-soft dark:aria-invalid:border-crit dark:aria-invalid:ring-crit-soft disabled:opacity-60';
...
export function Field({ id, label, hint, error, children, className }: FieldProps) {
  return (
    <div className={cn('flex min-w-0 flex-col gap-1.5', className)}>
      <Label htmlFor={id} className="text-[11.5px] font-medium text-text-2">
        {label}
      </Label>
      {children}
      {error ? (
        <p id={`${id}-error`} className="text-[11.5px] leading-snug text-crit">
          {error}
        </p>
      ) : hint ? (
        <p id={`${id}-hint`} className="text-[11.5px] leading-normal text-text-3">
          {hint}
        </p>
      ) : null}
    </div>
